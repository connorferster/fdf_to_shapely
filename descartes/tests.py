...
class PolygonTestCase(unittest.TestCase):
    ...
    def test_patch(self):
        ...
        self.failUnlessEqual(str(type(patch)), "<class 'matplotlib.patches.PathPatch'>")
        path = patch.get_path()
...
class GeoInterfacePolygonTestCase(unittest.TestCase):
    ...
    thing = GeoThing()
    thing.__geo_interface__ = (
        Point(0, 0)
        .buffer(10.0)
        .difference(MultiPoint([(-5, 0), (5, 0)]).buffer(3.0))
        .__geo_interface__
    )
...
class MultiPolygonTestCase(unittest.TestCase):
    ...
    polygon = (
        ...
        .buffer(10.0)
        .difference(MultiPoint([(-5, 0), (5, 0)]).buffer(3.0))
        .union(MultiPoint([(-10, 10), (10, -10)]).buffer(2.0))
    )
